...
# Description: Represent reconstruction API ressource.
...
@register
class Reconstruction(Ressource):
    _api_version = "v1"
    _name = "reconstruction"
    _primary_keys = ("id_reconstruction", "id_malette")

    class _rel:
        campaign = Relationship(RessourceProxy("campaign"))
        shots = Relationship(RessourceProxy("shot"), many=True)
        ref_lla = Relationship(RessourceProxy("sensors"))
